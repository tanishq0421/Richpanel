...
Shifts that cross midnight (e.g. 22:00–06:00) are normalized at write time into two ordinary same-day rows: a **primary** row ending at `24:00:00` on the starting weekday, and a **tail** row starting at `00:00:00` on the next weekday (`is_overnight_tail = true`). `24:00:00` (not `23:59:59`) is used specifically so the two rows are exactly adjacent with no gap and exact-hour duration math.
...
CREATE TABLE schedule_weekday_hours (
  ...
  weekday            smallint NOT NULL CHECK (weekday BETWEEN 0 AND 6),  -- 0=Monday..6=Sunday (arbitrary convention, not yet confirmed with stakeholders)
  start_time         time NOT NULL,
  end_time           time NOT NULL,          -- may be exactly '24:00:00' for an overnight primary row
  ...
  CHECK (NOT is_overnight_tail OR start_time = '00:00:00')
);
